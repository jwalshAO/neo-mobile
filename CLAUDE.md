...
- **Edge Function changes** (`supabase/functions/neo-mobile-chat/*.ts`):
  ```
  ./bin/deploy-edge.sh
  ```
  Calls `supabase functions deploy neo-mobile-chat` which uploads index.ts + caldav.ts. No Vercel rebuild needed.

  Requires one-time CLI auth (see `bin/deploy-edge.sh` header) — token at supabase.com/dashboard/account/tokens.

  **Avoid the `deploy_edge_function` MCP tool** unless the CLI isn't available. MCP requires embedding the entire file as a JSON parameter on every deploy, which is slow and expensive once the function gets large.
